  ...
  async (req, res) => {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (!title || !category || !code) {
        results.push({ row: rowNum, success: false, error: 'Missing title, category, or code' });
        continue;
      }

      try {
        await query(
          `INSERT INTO catalogue_items (title, category, code) VALUES ($1, $2, $3)`,
          [title, category, code]
        );
        results.push({ row: rowNum, success: true, code });
      } catch (err) {
        if (err.code === '23505') {
          results.push({ row: rowNum, success: false, error: `Code "${code}" already exists` });
        } else {
          results.push({ row: rowNum, success: false, error: 'Database error' });
        }
      }
    }
...
router.post('/loans-return', requireAuth, requireRole('librarian'), async (req, res) => {
  const { loan_ids } = req.body;

  if (!Array.isArray(loan_ids) || loan_ids.length === 0) {
    return res.status(400).json({ error: 'loan_ids must be a non-empty array' });
  }
  ...
  for (const loanId of loan_ids) {
    try {
      ...
      if (!loan) {
        results.push({ loan_id: loanId, success: false, error: 'Loan not found' });
        continue;
      }
      ...
      await logEvent(loanId, 'returned', req.user.id, 'Returned via bulk action');

      results.push({ loan_id: loanId, success: true });
    } catch (err) {
      results.push({ loan_id: loanId, success: false, error: 'Database error' });
    }
  }
...
router.get('/loans-export', requireAuth, requireRole('librarian'), async (req, res) => {
  try {
    ...
    const csv = stringify(result.rows, {
      header: true,
      columns: ['item_title', 'item_code', 'borrower_name', 'borrower_email', 'due_date'],
    });

    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', 'attachment; filename="items-on-loan.csv"');
    return res.send(csv);
  } catch (err) {
    console.error('Export loans error:', err);
    return res.status(500).json({ error: 'Something went wrong exporting loans' });
  }
});
